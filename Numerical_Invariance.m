...
f = 1 + zeros(1,n+1);
%grd=(0:h:1); % Equally spaced points
grd=(1+cos((0:n)*pi./n))./2; % Chebyshev nodes for [0,1]
...
%for m = 1:10
    ...
    for i = 1:steps
        f = fNew(f,n,grd);
        disp(['Approx. integral value:', num2str(trapz((0:h:1),f))]);
...
%[C,p]=my_loglog(Nvec,errvec,'n','Error','Error over n, Cheb. nodes');
